...
import { normalizeHelloFresh } from '../_lib/normalize.js';
...
export default async function handler(req, res) {
  ...
  let recipes = [];
  if (site === 'hellofresh') recipes = await scrapeHelloFresh();
  else if (site === 'marleyspoon') recipes = await scrapeMarleySpoon();
  else if (site === 'nyt') recipes = await scrapeNYT();
  ...
  if (recipes.length > 0) {
    const { error } = await supabase
      ...
      .upsert(
        recipes.map((r) => ({ id: r.id, source: r.source, data: r, scraped_at: new Date().toISOString() })),
        { onConflict: 'id' }
      );
...
async function scrapeMarleySpoon() {
  ...
  return (data.items || []).map((item) => {
    ...
    return {
      ...
      keywords: [],
      macros: { protein: f.nutritionFacts?.protein || 0, carbs: f.nutritionFacts?.carbohydrates || 0, fat: f.nutritionFacts?.fat || 0, calories: f.nutritionFacts?.calories || 0 },
      steps: (f.steps || []).map((s) => s?.fields?.description || ''),
      ingredients: (f.ingredients || []).map((i) => ({ name: i?.fields?.ingredient?.fields?.name || '', amount: `${i?.fields?.amount || ''} ${i?.fields?.unit || ''}`.trim() })),
    };
  });
}
...
async function scrapeNYT() {
  ...
  const res = await fetch('https://cooking.nytimes.com/search?q=', {
    headers: { 'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X)', Cookie: cookie },
  });
  if (!res.ok) return [];
    ...
    try {
      ...
      for (const item of items) {
        ...
        recipes.push({
          ...
          macros: { calories: parseInt(item.nutrition?.calories) || 0, protein: parseInt(item.nutrition?.proteinContent) || 0, carbs: parseInt(item.nutrition?.carbohydrateContent) || 0, fat: parseInt(item.nutrition?.fatContent) || 0 },
          steps: (item.recipeInstructions || []).map((s) => typeof s === 'string' ? s : s.text || ''),
