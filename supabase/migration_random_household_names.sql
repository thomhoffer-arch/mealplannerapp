-- Give new households a fun, random food/cooking related name.
-- Updates create_household_for_user to pick one automatically when
-- the row has no name yet, and creates a helper function.

create or replace function public.random_household_name()
returns text
language sql
as $$
  select (array[
...
-- Update the RPC to name new households automatically.
...
begin
  ...
  values (public.random_household_name(), uid)
